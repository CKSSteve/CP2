...
<script type="text/javascript" language="javascript">
	...
	function quotionEntry(varTenderNo) {
    	...
    	$("#queryForm").attr("action", '<c:url value="/quotion/quotionEntry"/>');
    	$("#queryForm").submit();
	}
...
<body id="scroller">
	...
	<h1><span class="title-l">待報價清冊</span></h1>
	...
	<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <c:if test="${quotionEntryList.totalElements > 0}">	     
	      <c:forEach items="${quotionEntryList.content }" var="item" varStatus="idx" >
		...
		<c:if test="${quotionEntryList.totalElements == 0}">
			...
			<!-- 分頁  -->
			<c:if test="${quotionEntryList.totalPages > 1}">
				<table width="98%" border="0" align="center" cellpadding="0"
					cellspacing="0" id="table-count">
					<tr>
						<tags:pagination page="${quotionEntryList}" paginationSize="5" />
					</tr>
				</table>
			</c:if>
			<!-- 分頁結束 -->

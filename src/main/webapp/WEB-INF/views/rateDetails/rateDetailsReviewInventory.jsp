...
<script type="text/javascript" language="javascript">
	...
	function rateDetailsReview(varQuationNo,varItemNo) {
		$("#quationNo").val(varQuationNo);
		$("#itemNo").val(varItemNo);
    	$("#queryForm").attr("action", '<c:url value="/rateDetails/rateDetailsReview"/>');
    	$("#queryForm").submit();
	}
...
<body id="scroller">
	...
	<h1><span class="title-l">本票帳務明細待審核清冊</span></h1>
      ...
      <c:if test="${awardDataList.totalElements > 0}">	     
	      <c:forEach items="${awardDataList.content }" var="item" varStatus="idx" >
		      ...
		      <td align="center" nowrap="nowrap"><c:out value="${idx.index+1}"/></td>
		        <td align="center" nowrap="nowrap"><a href="#" onclick="rateDetailsReview('<c:out value="${item.id.quationNo}"/>','<c:out value="${item.id.itemNo}"/>')"><c:out value="${item.batchNo}"/></a></td>

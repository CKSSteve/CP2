...
$(function() {
	...
	$("#quotesDateEndDate").datepicker({showOn: 'both', showOtherMonths: true,
   	...
   	$("#query").click(function(){		
		...
		if(quotesStartDate != ""){
			var startDate = new Date(quotesStartDate).getTime();			
		}
		
		if(quotesEndDate != ""){
			var endDate = new Date(quotesEndDate).getTime();			
		}
		
		if (quotesStartDate.length != 0 && quotesEndDate.length != 0 && quotesStartDate > quotesEndDate) {
			alert("報價起始日期不能小於結束日期");
			return false;
		}else if(quotesStartDate.length == 0 && quotesEndDate.length == 0){
			alert("請輸入報價日期");
			return false;
		}else if(quotesStartDate.length == 0 || quotesEndDate.length == 0){
			alert("起迄日皆需輸入");
			return false;
		} else if(quotesStartDate.length != 0 && quotesEndDate.length != 0 && ( (endDate - startDate)/(1000 * 60 * 60 * 24) > 365 ) ){
			alert("報價日期查詢區間以一年為限");
			return false;
	    }
			$("#queryForm").attr("action", '<c:url value="/quotionQuery/doQuery"/>');
			$("#queryForm").submit();
	});
   	
   	$("#reset").click(function(){
   		
   		$("#quotesDateStartDate").val("");
   		$("#quotesDateEndDate").val("");
   		$("#issuerId").val("0");
   		$("#currencyId").val("0");  
   		$("input[name='statusTypeArray']").prop("checked",false);
   		$("input[name='approveStatusTypeArray']").prop("checked",false);
   	});
})

function showTenderDetail(varTenderNo){
	$("#detailTenderNo").val(varTenderNo);
	$("#method").val("/quotionQuery/doQuery");
...
function showQuationDetail(varQuationNo){
	...
	$("#method").val("/quotionQuery/doQuery");
	$("#queryForm").attr("action", '<c:url value="/quotionQuery/showDetail"/>');
	$("#queryForm").submit();
}
      ...
      <c:if test="${queryQuotesList.totalElements > 0}">	     
	      <c:forEach items="${queryQuotesList.content }" var="item" varStatus="idx" >
		      <tr class="<c:out value="${idx.index%2 eq 0 ? '' : 'td-lighty' }" />">
		        ...
		        <td align="center" nowrap="nowrap"><fmt:formatDate value="${item.quationTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
